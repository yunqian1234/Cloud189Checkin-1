...
const pushServerChan = (title, desp) => {
    ...
    superagent
      .post(`https://sctapi.ftqq.com/${process.env.SERVERCHAN_SENDKEY}.send`)
      .type("form")
      .send(data)
      .end((err, res) => {
            if (err) {
...
const pushTelegramBot = (title, desp) => {
    if (!(process.env.TELEGRAM_BOTTOKEN && process.env.TELEGRAM_CHATID)) {
        return;
    }
    const data = {
    ...
    superagent
      .post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOTTOKEN}/sendMessage`)
      .type("form")
      .send(data)
      .end((err, res) => {
            if (err) {
...
const pushWecomBot = (title, desp) => {
    if (!(process.env.WECOM_BOTKEY && process.env.WECOM_TELPHONE)) {
        return;
    }
    const data = {
        msgtype: "text",
        text: {
            content: `${title}\n\n${desp}`,
            mentioned_mobile_list: [process.env.WECOM_TELPHONE],
        },
    };
    superagent
      .post(
            `https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=${process.env.WECOM_BOTKEY}`
        )
      .send(data)
      .end((err, res) => {
            if (err) {
